 ...
 */
public class HokageIconFactory implements Serializable {
    ...
    private static class FileChooserDetailViewIcon implements Icon, UIResource, Serializable {
	    ...
	    g.setColor(HokageLookAndFeel.getPrimaryControlInfo());
	    //     top
	    ...
	    g.setColor(HokageLookAndFeel.getPrimaryControlHighlight());
    ...
    private static class FileChooserNewFolderIcon implements Icon, UIResource, Serializable {
	    ...
	    // Draw tab on folder
	    g.setColor(HokageLookAndFeel.getPrimaryControlDarkShadow());
    ...
    public static class PaletteCloseIcon implements Icon, UIResource, Serializable{
	    ...
	    Color back;
	    Color highlight = HokageLookAndFeel.getPrimaryControlHighlight();
	    Color shadow = HokageLookAndFeel.getPrimaryControlInfo();
	    if (buttonModel.isPressed() && buttonModel.isArmed()) {
	        back = shadow;
	    } else {
	        back = HokageLookAndFeel.getPrimaryControlDarkShadow();
	    }
	...
	int iconSize = 16;
	    ...
	    Color backgroundColor = HokageLookAndFeel.getPrimaryControl();
	    Color internalBackgroundColor = 
		HokageLookAndFeel.getPrimaryControl();
	    Color mainItemColor = 
		HokageLookAndFeel.getPrimaryControlDarkShadow();
	    Color darkHighlightColor = HokageLookAndFeel.getBlack();
	    Color xLightHighlightColor = HokageLookAndFeel.getWhite();
	    Color boxLightHighlightColor = HokageLookAndFeel.getWhite();
	    
	    // if the inactive window
	    if (parentButton.getClientProperty("paintActive") != Boolean.TRUE)
	    {
		backgroundColor = HokageLookAndFeel.getControl();
		internalBackgroundColor = backgroundColor;
		mainItemColor = HokageLookAndFeel.getControlDarkShadow();
	...
	int iconSize = 16;
	    ...
	    Color ulLightHighlightColor = HokageLookAndFeel.getWhite();
	    Color lrLightHighlightColor = HokageLookAndFeel.getWhite();
    ...
    private static class InternalFrameDefaultMenuIcon implements Icon, UIResource, Serializable {
	    ...
	    Color windowBodyColor = HokageLookAndFeel.getWindowBackground();
	    Color titleColor = HokageLookAndFeel.getPrimaryControl();
	    Color edgeColor = HokageLookAndFeel.getPrimaryControlDarkShadow();
	...
	private void paintOceanIcon(Component c, Graphics g, int x, int y) {
		...
		if (model.isPressed() && model.isArmed()) {
                    ...
                    HokageUtils.drawGradient(c, g, "CheckBox.gradient", 0, 0,
                                            w, h, true);
                    g.setColor(HokageLookAndFeel.getControlDarkShadow());
		...
		g.setColor( HokageLookAndFeel.getControlInfo() );
	    } else {
                g.setColor(HokageLookAndFeel.getControlDarkShadow());
                g.drawRect(0, 0, w - 1, h - 1);
	    }
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		if (model.isPressed() && model.isArmed()) {
		    ...
		    HokageUtils.drawPressed3DBorder(g, x, y, controlSize, controlSize);
		} else {
		    HokageUtils.drawFlush3DBorder(g, x, y, controlSize, controlSize);
		}
		g.setColor( HokageLookAndFeel.getControlInfo() );
	    } else {
	        g.setColor( HokageLookAndFeel.getControlShadow() );
    ...
    private static class RadioButtonIcon implements Icon, UIResource, Serializable {
	    ...
	    g.translate(x, y);
            if (enabled && !pressed) {
                ...
                HokageUtils.drawGradient(c, g, "RadioButton.gradient",
                                        1, 1, 10, 10, true);
	    ...
	    Color shadow = HokageLookAndFeel.getControlShadow();
	    Color darkCircle = HokageLookAndFeel.getControlDarkShadow();
	    Color whiteInnerLeftArc = HokageLookAndFeel.getControlHighlight();
	    Color whiteOuterRightArc = HokageLookAndFeel.getControlHighlight();
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		// Draw fill
		g.setColor( HokageLookAndFeel.getWindowBackground() );
		g.fillRect( 4, 2, 9, 12 );
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
	    ...
	    if (image == null || cachedOrientation != HokageUtils.isLeftToRight(c)) {
		cachedOrientation = HokageUtils.isLeftToRight(c);
		if (gc != null) {
		    image = gc.createCompatibleImage(getIconWidth(), 
						     getIconHeight(),
						     Transparency.BITMASK);
	    ...
	    if (HokageUtils.isLeftToRight(c)) {
	...
	public void paintMe(Component c, Graphics g, int x, int y) {
	    ...
	    int xoff = (HokageUtils.isLeftToRight(c)) ? 0 : 4;
  ...
  //
	...
	public void paintIcon( Component c, Graphics g, int x, int y )
	{
	    ...
	    if ( !model.isEnabled() )
	    {
	        g.setColor( HokageLookAndFeel.getMenuDisabledForeground() );
	    }
	    else
	    {
	        if ( model.isArmed() || ( c instanceof JMenu && model.isSelected() ) )
		{
		    g.setColor( HokageLookAndFeel.getMenuSelectedForeground() );
		}
		else
		{
            ...
            if( HokageUtils.isLeftToRight(b) ) {
                g.drawLine( 0, 0, 0, 7 );
                g.drawLine( 1, 1, 1, 6 );
                g.drawLine( 2, 2, 2, 5 );
                g.drawLine( 3, 3, 3, 4 );
	...
	public void paintOceanIcon(Component c, Graphics g, int x, int y) {
	    ...
	    g.translate(x, y);
            if (isEnabled) {
                HokageUtils.drawGradient(c, g, "CheckBoxMenuItem.gradient",
                                        1, 1, 7, 7, true);
	    ...
	    if (isSelected) {
		    ...
		    if (isArmed || ( c instanceof JMenu && isSelected)) {
		        g.setColor(
                            HokageLookAndFeel.getMenuSelectedForeground() );
		    }
		    else {
		        g.setColor(c.getForeground());
		    }
	...
	public void paintOceanIcon(Component c, Graphics g, int x, int y) {
	    ...
	    g.translate( x, y );
            ...
            if (isEnabled) {
                HokageUtils.drawGradient(c, g, "RadioButtonMenuItem.gradient",
                                        1, 1, 7, 7, true);
	        if (isPressed || isArmed) {
		    g.setColor(HokageLookAndFeel.getPrimaryControl());
                }
...
private static class VerticalSliderThumbIcon implements Icon, Serializable, UIResource {
    protected static HokageBumps controlBumps;
    protected static HokageBumps primaryBumps;

    public VerticalSliderThumbIcon() {
        controlBumps = new HokageBumps( 6, 10,
				HokageLookAndFeel.getControlHighlight(),
				HokageLookAndFeel.getControlInfo(),
				HokageLookAndFeel.getControl() );        
        primaryBumps = new HokageBumps( 6, 10,
				HokageLookAndFeel.getPrimaryControl(),
				HokageLookAndFeel.getPrimaryControlDarkShadow(),
				HokageLookAndFeel.getPrimaryControlShadow() );        
    }
	...
	if ( slider.hasFocus() ) {
	    ...
	    g.setColor( slider.isEnabled() ? HokageLookAndFeel.getPrimaryControlInfo() :
	...
	// Draw the highlight
	if ( slider.isEnabled() ) {
	    g.setColor( slider.hasFocus() ? HokageLookAndFeel.getPrimaryControl()
			: HokageLookAndFeel.getControlHighlight() );
	    if (leftToRight) {
	        g.drawLine( 1, 1, 8, 1 );
		g.drawLine( 1, 1, 1, 13 );
...
private static class HorizontalSliderThumbIcon implements Icon, Serializable, UIResource {
    ...
    public HorizontalSliderThumbIcon() {
        controlBumps = new HokageBumps( 10, 6,
				HokageLookAndFeel.getControlHighlight(),
				HokageLookAndFeel.getControlInfo(),
				HokageLookAndFeel.getControl() );        
        primaryBumps = new HokageBumps( 10, 6,
				HokageLookAndFeel.getPrimaryControl(),
				HokageLookAndFeel.getPrimaryControlDarkShadow(),
				HokageLookAndFeel.getPrimaryControlShadow() );        
    }
    ...
    private static class OceanVerticalSliderThumbIcon extends CachedPainter
        ...
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            paint(c, g, x, y, getIconWidth(), getIconHeight(),
                  HokageUtils.isLeftToRight(c), c.hasFocus(), c.isEnabled(),
                  HokageLookAndFeel.getCurrentTheme());
        }
        ...
        protected void paintToImage(Component c, Graphics g2,
            ...
            if (!enabled) {
                ...
                HokageUtils.drawGradient(c, g, "Slider.focusGradient",
                                        1, 1, 14, 14, false);
            }
            else {
                HokageUtils.drawGradient(c, g, "Slider.gradient",
                                        1, 1, 14, 14, false);
            }
            g.setClip(clip);
            ...
            if (hasFocus) {
                ...
                g.setColor(enabled ? HokageLookAndFeel.getPrimaryControlInfo() :
                           HokageLookAndFeel.getControlDarkShadow());
            }
