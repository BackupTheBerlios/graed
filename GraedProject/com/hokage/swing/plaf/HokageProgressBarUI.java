 ...
 */
public class HokageProgressBarUI extends BasicProgressBarUI {

    private Rectangle innards;
    private Rectangle box;

    public static ComponentUI createUI(JComponent c) {
	return new HokageProgressBarUI();
    }
	...
	if (progressBar.isBorderPainted()) {
	    ...
	    int amountFull = getAmountFull(b, barRectWidth, barRectHeight);
            boolean isLeftToRight = HokageUtils.isLeftToRight(c);
		...
		g2.setColor(HokageLookAndFeel.getControlShadow());
                g2.drawLine(startX, startY, endX, startY);

                if (amountFull > 0) {
                    // Draw darker lengthwise line over filled area.
		    g2.setColor(HokageLookAndFeel.getPrimaryControlDarkShadow());
