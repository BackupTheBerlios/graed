...
public class HokageBorders {
    ...
    public static class Flush3DBorder extends AbstractBorder implements UIResource{
        ...
        public void paintBorder(Component c, Graphics g, int x, int y,
			  int w, int h) {
            if (c.isEnabled()) {
                HokageUtils.drawFlush3DBorder(g, x, y, w, h);
            } else {
                HokageUtils.drawDisabledBorder(g, x, y, w, h);
            }
        }
       ...
       protected static Insets borderInsets = new Insets( 3, 3, 3, 3 );
	    ...
	    if ( model.isEnabled() ) {
	        ...
	        if (isPressed && isDefault) {
                    HokageUtils.drawDefaultButtonPressedBorder(g, x, y, w, h);
                } else if (isPressed) {
                    HokageUtils.drawPressed3DBorder( g, x, y, w, h );
	        } else if (isDefault) {
                    HokageUtils.drawDefaultButtonBorder( g, x, y, w, h, false);
                } else {
                    HokageUtils.drawButtonBorder( g, x, y, w, h, false);
	        }
	    } else { // disabled state
	        HokageUtils.drawDisabledBorder( g, x, y, w-1, h-1 );
	    }
        }
    ...
    public static class InternalFrameBorder extends AbstractBorder implements UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y,
            ...
            if (c instanceof JInternalFrame && ((JInternalFrame)c).isSelected()) {
                background = HokageLookAndFeel.getPrimaryControlDarkShadow();
	        highlight = HokageLookAndFeel.getPrimaryControlShadow();
	        shadow = HokageLookAndFeel.getPrimaryControlInfo();
            } else {
                background = HokageLookAndFeel.getControlDarkShadow();
	        highlight = HokageLookAndFeel.getControlShadow();
	        shadow = HokageLookAndFeel.getControlInfo();
            }
    ...
    {		
        ...
        {
            return HokageLookAndFeel.getPrimaryControlDarkShadow();
        }

        protected Color getActiveHighlight()
        {
            return HokageLookAndFeel.getPrimaryControlShadow();
        }

        protected Color getActiveShadow()
        {
            return HokageLookAndFeel.getPrimaryControlInfo();
        }

        protected Color getInactiveBackground()
        {
            return HokageLookAndFeel.getControlDarkShadow();
        }

        protected Color getInactiveHighlight()
        {
            return HokageLookAndFeel.getControlShadow();
        }

        protected Color getInactiveShadow()
        {
            return HokageLookAndFeel.getControlInfo();
        }
    ...
    public static class OptionDialogBorder extends AbstractBorder implements UIResource {
	    ...
	    switch (messageType) {
            ...
            default:
                borderColor = HokageLookAndFeel.getPrimaryControlDarkShadow();
                break;
	    }
    ...
    public static class MenuItemBorder extends AbstractBorder implements UIResource {
	    ...
	    if ( c.getParent() instanceof JMenuBar ) {
	        if ( model.isArmed() || model.isSelected() ) {
	            ...
	            g.setColor( HokageLookAndFeel.getMenuBackground() );
    ...
    public static class ToolBarBorder extends AbstractBorder implements UIResource, SwingConstants
    {
         ...
         */
        private static final long serialVersionUID = 1L;
        protected HokageBumps bumps = new HokageBumps( 10, 10,
        ...
        public void paintBorder( Component c, Graphics g, int x, int y, int w, int h )
	{
		...
		{
                    int shift = HokageLookAndFeel.usingOcean() ? -1 : 0;
	...
	if (textBorder == null) {
	    textBorder = new BorderUIResource.CompoundBorderUIResource(
						   new HokageBorders.Flush3DBorder(),
						   new BasicBorders.MarginBorder());
	}
	return textBorder;
    }
	...
	if (textFieldBorder == null) {
	    textFieldBorder = new BorderUIResource.CompoundBorderUIResource(
						   new HokageBorders.TextFieldBorder(),
						   new BasicBorders.MarginBorder());
	}
	...
	if (toggleButtonBorder == null) {
	    toggleButtonBorder = new BorderUIResource.CompoundBorderUIResource(
						   new HokageBorders.ToggleButtonBorder(),
						   new BasicBorders.MarginBorder());
	}
	    ...
	    ButtonModel model = button.getModel();
            if (HokageLookAndFeel.usingOcean()) {
                if(model.isArmed()) {
                    ...
                    HokageUtils.drawPressed3DBorder( g, x, y, w + 1, h + 1 );
                }
                return;
            }
	    if (! c.isEnabled() ) {
		    ...
		    HokageUtils.drawDark3DBorder( g, x, y, w, h );
                } else {
		    HokageUtils.drawFlush3DBorder( g, x, y, w, h );
                }
	    }
	...
	return new BorderUIResource.CompoundBorderUIResource(
                                          new LineBorder(HokageLookAndFeel.getControlDarkShadow(), 1),
                                          new MatteBorder (2,2,1,2, HokageLookAndFeel.getControl()));
    }
    ...
    static Border getToolBarRolloverBorder() {
        if (HokageLookAndFeel.usingOcean()) {
            return new CompoundBorder(
                new HokageBorders.ButtonBorder(), 
                new HokageBorders.RolloverMarginBorder());
        }
	return new CompoundBorder(new HokageBorders.RolloverButtonBorder(), 
				  new HokageBorders.RolloverMarginBorder());
    }
	...
	return new CompoundBorder(new HokageBorders.ButtonBorder(),
				  new HokageBorders.RolloverMarginBorder());
    }
}
