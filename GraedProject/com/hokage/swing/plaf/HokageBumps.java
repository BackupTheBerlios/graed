...
class HokageBumps implements Icon {
    ...
    public HokageBumps( Dimension bumpArea ) {
        this( bumpArea.width, bumpArea.height );
    }

    public HokageBumps( int width, int height ) {
        this(width, height, HokageLookAndFeel.getPrimaryControlHighlight(),
    ...
    private void createImage() {
        if (gc != null) {
            image = gc.createCompatibleImage(IMAGE_SIZE, IMAGE_SIZE,
                       (backColor != HokageBumps.ALPHA) ? Transparency.OPAQUE :
            ...
            IndexColorModel icm = new IndexColorModel(8, 3, cmap, 0, false,
                      (backColor == HokageBumps.ALPHA) ? 0 : -1,
